---
title: VCV Rack Modules Manual
---

Surge XT modules for VCV Rack is a collection of modules which brings the majority of the DSP code
...
<iframe
    ...
    src="https://www.youtube-nocookie.com/embed/8MavXLbI50A?si=QrP9uBBgCLRZUsLe"
...
![An Image of some Rack Modules](/images/rack-xt-manual/overview.png)
...
![A Sine VCO Module](/images/rack-xt-manual/basic_module.png)

![A Flanger Module](/images/rack-xt-manual/flanger_basic.png)
...
Modulation inputs do _not_ set polyphony in modules. Instead, a module which has
...
4. P=n; M=m, m!=n; In this case the first m modulator channels are used for
...
![](/images/rack-xt-manual/skins.PNG)

Here are some examples of the base skins:

![Darkl Theme](/images/rack-xt-manual/darktheme.PNG)

![Medium Theme](/images/rack-xt-manual/mediumtheme.PNG)

![Light Theme](/images/rack-xt-manual/lighttheme.PNG)
...
![Modern VCO](/images/rack-xt-manual/modern.PNG)

![Wavetable VCO](/images/rack-xt-manual/wavetable.PNG)

![Twist VCO](/images/rack-xt-manual/twist.PNG)
...
![The Wavetable menu](/images/rack-xt-manual/wavetable_menu.png)
...
# The VCF, Waveshaper and Digital Combinator Modules
...
![VCF Module](/images/rack-xt-manual/filter.png)
...
![Waveshaper Module](/images/rack-xt-manual/waveshaper.PNG)

## Digital Combinator / CXOR

The Digital Combinator module provides two independent polyphonic circuits which can use a variety of digital
signal combination techniques to add texture and growl to a pair of signals. The basic setting 'Ring Modulator',
upsamples the signal, multiplies a * b digitally, downsamples, and sends it to the output. As a result this
module has a latency of 4 samples, the minimum block size for the Surge halfband up/down filters.

The module also features a large number of other combination functions, though, each with a unique and signal
dependent character. The best approach is to just try them and see what sounds good!

Finally, the module allows a double-cascade. If the second input "A" input has the power button between
the inputs activated, the second circuit will use the output of the first as its input without the cost
of an additional up/down sample, allowing for a wide variety of three-signal combinations.

![Digital Combinator](/images/rack-xt-manual/cxor.png)
...
![Chorus Module](/images/rack-xt-manual/chorus.PNG)
...
![Polyphony Selection](/images/rack-xt-manual/polyphony.PNG)
...
the cables will _very slightly_ lengthen the delay time. This would make the module play out-of-tune in
...
![Basic Tuned Delay+ Patch](/images/rack-xt-manual/Tuned%20Delay+.png)
...
![LFOxEG Module](/images/rack-xt-manual/lfoxeg.PNG)

There are a number of menu options.

-   _Polyphony_ works the same as everywhere else
-   _Envelope Triggers From Zero_ makes the envelope restart from zero when triggered. Defaults to off, meaning envelopes will restart from the current value.
-   _Random Phase on Attack_ is fairly self-explanatory.
-   _Scale LFO and EG outputs by Amp_ defaults to on, meaning the Amp knob affects LFO, EG and LFOxEG outputs. Turn off to make Amp affect LFOxEG output only.
-   _Set EG to Zero when No Trigger Connected_ sets whether the EG output defaults to open or closed when nothing is patched to the gate inputs.
-   _Rack Randomization Changes Shape_ defaults to on. Turn this off to randomize parameters while staying on the same LFO type.
...
![EGxVCA Module](/images/rack-xt-manual/EGxVCA.png)
...
![Quad AD Module](/images/rack-xt-manual/Quad%20AD.png)
...
![Quad LFO Module](/images/rack-xt-manual/Quad%20LFO.png)
...
A complete wiring, used to make two BogAudio PWM VCOs participate in polyphonic unison with a PWM sweep, is shown here.

![Unison Module Example](/images/rack-xt-manual/unisonex.png)
...
![Mod Matrix Module](/images/rack-xt-manual/modmatrix.PNG)
