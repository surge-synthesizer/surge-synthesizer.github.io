...
| **block_size**      | Audio buffer size in samples used by Surge XT                                          |                                       |
...
| **cc_sus**          | Sustain pedal signal                                                                   | Toggles between 0 or 1                |
| **chan_at**         | Monophonic aftertouch if MPE is disabled                                               | Ranges from 0 to 1                    |
| **clamp_output**    | Can be set to false to allow the modulator to disable the output range limit           |                                       |
...
| **is_rendering_ui** | Is set to true if the call is used to update the graph in the UI                       |                                       |
...
| **pb**              | Pitchbend wheel value                                                                  | Ranges from -1 to 1                   |
| **pb_range_dn**     | Pitchbend depth down                                                                   | Ranges from 0 to 24                   |
| **pb_range_up**     | Pitchbend depth up                                                                     | Ranges from 0 to 24                   |
...
| **retrigger_AEG**   | Can be set to true to trigger the Amplitude Envelope Generator                         |                                       |
| **retrigger_FEG**   | Can be set to true to trigger the Filter Envelope Generator                            |                                       |
...
|                   |                                                      |                      |
| ----------------- | ---------------------------------------------------- | -------------------- |
| **channel**       | MIDI or MPE channel that triggered the modulator     |                      |
| **key**           | Key that triggered the modulator                     | MIDI note number     |
| **mpe_bend**      | MPE pitchbend                                        |                      |
| **mpe_bendrange** | MPE pitchbend range                                  |                      |
| **mpe_timbre**    | MPE timbre                                           |                      |
| **poly_at**       | Per note polyphonic aftertouch                       | Ranges from 0 to 1   |
| **poly_limit**    | Polyphony limit                                      |                      |
| **rel_velocity**  | Per note release velocity amount                     | Ranges from 0 to 127 |
| **velocity**      | Per note velocity amount                             | Ranges from 0 to 127 |
| **voice_count**   | Number of currently active voices                    |                      |
| **voice_id**      | Integer value that gets incremented on each new note |                      |
