...
#### Start/Stop OSC connections
As its name suggests, clicking this option toggles the OSC connection to Surge XT on and off. A connection will
be created according to the port settings below it.

#### Change OSC input port
Allows you to change the port used for incoming OSC data. The currently used port is displayed in parentheses.

#### Change OSC output port
Allows you to change the port used for outgoing OSC data. The currently used port is displayed in parentheses.
