...
- **OSC In**: Allows you to change the port used for incoming OSC data. The currently used port is displayed in parentheses. Click on
  *Default Port* to reset the input port to its default value.
  ...
  If the requested input port is already in use (for example, by another instance of Surge XT), Surge automatically
  tries the next 100 ports and binds to the first one available. This allows multiple instances of Surge to receive OSC on the
  same machine. When running as a plugin, Surge saves the port it was last bound to with the project and tries to restore it first.
  The standalone always starts from the configured port.
...
- **OSC Out**: Allows you to change the port used for outgoing OSC data. The currently used port is displayed in parentheses. Click on
  *Default Port* to reset the output port to its default value.
  ...
  When the output IP address points to another machine, Surge also sends *from* this port number, so the receiving end can
  identify Surge by its source port. This isn't possible when sending to the loopback address, in which case the operating
  system assigns an arbitrary source port instead.

- **Out IP Address**: Allows you to change the output IP address used for the OSC protocol. Click on *Local Host* to reset the output
  IP address to its default value (the loopback address).

- **?**: Opens this section of the manual or displays the OSC specification (see below).

A status line at the bottom of the window shows which port Surge is actually listening on, and notes in parentheses when that
port was restored from a previous session or when the requested one was already in use.

#### Show OSC Specification...

Clicking on this option will open a local page in your browser that will provide information about the specific
implementation of OSC in Surge XT, as well as some more general information and documentation about the OSC
protocol. You can also read the [OSC specification](https://html-preview.github.io/?url=https://github.com/surge-synthesizer/surge/blob/main/resources/surge-shared/oscspecification.html) online.
