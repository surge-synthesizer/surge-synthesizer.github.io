...
The Wavetable Script Editor (WTSE) is a Lua-powered scripting environment for procedurally generating wavetables. Unlike the Formula modulator, which runs continuously on each sample block in real time, the WTSE outputs only when you press Generate. The result is a new wavetable set directly onto the corresponding Wavetable oscillator, which can then sweep through its frames via the `Morph` parameter like any other wavetable.
...
You may add any fields you like to `wt` and return it; everything you store here will be available in every subsequent generate call, an optional field is `wt.name` which sets the Wavetable for the Oscillator.
...
As with the Formula modulator, a wavetable-specific version of the Prelude is available in Wavetable Scripts. This provides a set of built-in utilities, including:
...
Standard math functions such as sin, cos, pi, exp, and log are available in the global scope without the math. prefix.
...
The graph in the UI can be switched between single-frame and filmstrip mode to preview the wavetable. Dragging it moves through frames; if the script is unchanged, frames are loaded from cache, otherwise `generate` is called for each frame as needed.
...
Real transforms operate on a flat array of real-valued (time-domain) samples:

`real_fft(t)` performs a forward real FFT on table `t`, returning a table of the same length. The first two elements of the output are the DC and Nyquist bins respectively, followed by interleaved real/imaginary pairs for each frequency bin.
...
For the real FFT output format, the bin layout is:
