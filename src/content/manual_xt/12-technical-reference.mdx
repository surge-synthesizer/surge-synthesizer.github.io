...
To load an impulse response, click on the box that says "No impulse response loaded!" You can pick one of the factory impulses from the drop-down menu, or load your own. Any loaded impulse response will be copied to the "IMPORTED" directory under the user content directory. You can also drag-and-drop a file into the box, which will load it as an impulse response. File formats supported are WAV and FLAC, with 1 or 2 channels of audio.

Like wavetables, impulse responses are saved as part of the Surge patch file and distributed with the patch.
...
| Reverse From* | Flips the IR, starting from the beginning, to the end (0%) or up to halfway through (50%) it.     | 0 .. 50 %, disableable                 |
| Tilt Center   | Center frequency of an integrated tilt EQ.                                                        | 13.75 .. 25087.71 Hz, disableable      |
| Tilt Slope    | Slope of the integrated tilt EQ; negative values slope downwards to the left.                     | -12 .. +12 dB                          |
| Low Cut       | Cutoff frequency of built-in highpass filter.                                                     | 13.75 .. 25087.71 Hz, disableable      |
| High Cut      | Cutoff frequency of built-in lowpass filter.                                                      | 13.75 .. 25087.71 Hz, disableable      |
| Mix           | Blend control between the dry and the wet signal.                                                 | -100 .. +100%                          |

Unlike most of Surge's FX suite, not every parameter here is modulatable. Parameters marked with an asterisk are not modulatable, and can only be changed manually. Some DAWs may be able to automate them, but we do not guarantee a functional sound if you do so.

The Convolution effect is not available in Surge FX; it only appears in the Surge Synthesizer directly.
