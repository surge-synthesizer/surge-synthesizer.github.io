...
| Feedback      | FM feedback amount.<br />Can be extended.<br />Can be set to Vintage FM (smoother feedback).                                                                           | -100 .. 100 %<br/>-400 .. 400 %                                      |
...
| Feedback    | Modulation amount of the carrier to itself<br />Extended mode (default) can be disabled.<br />Can be set to Vintage FM (smoother feedback).  | -400 .. 400 % <br/> -100 .. 100 % |
...
| M3 Frequency | Frequency of the third modulator                                                                                                            | ~ 14 Hz .. 25 kHz<br />0.5 Hz .. ~25 kHz             |
| Feedback     | Modulation amount of the carrier to itself<br />Extended mode (default) can be disabled.<br />Can be set to Vintage FM (smoother feedback). | -400 .. 400 % <br/> -100 .. 100 %                    |
...
It has stable self-oscillation and requires less CPU processing than the newer Vintage Ladder filter. Available in **Lowpass** type.
...
**OB-Xd 12dB** - 2-pole filters from the OB-Xd synthesizer originally by [Vadim Filatov (2DaT)](https://github.com/2DaT/Obxd), which is based on the filters found in the Oberheim OB-Xa.
Available in **Lowpass**, **Highpass**, **Bandpass** and **Notch** types.
...
**OB-Xd 24 dB** - 4-pole filters from the OB-Xd synthesizer originally by [Vadim Filatov (2DaT)](https://github.com/2DaT/Obxd), which is based on the filters found in the Oberheim OB-Xa.
Available in **Lowpass** type.
...
**Cutoff Warp** - 2-pole filters created using a non-linear biquad filter structure. These non-linearities cause the cutoff frequency to sweep to higher frequencies as the signal level increases (see below).
Available in **Lowpass**, **Highpass**, **Bandpass**, **Notch** and **Allpass (Effect)** types.

![Illustration 74: Cutoff warp frequency response](/images/manual-xt/Pictures/cutoff_warp.png)

For more information on the Cutoff Warp filter, see [this blog post](https://jatinchowdhury18.medium.com/complex-nonlinearities-episode-4-nonlinear-biquad-filters-ae6b3f23cb0e) by Jatin Chowdhury,
or [this 2020 DAFx paper](https://dafx2020.mdw.ac.at/proceedings/papers/DAFx2020_paper_3.pdf), specifically section 4.

**Resonance Warp** - 2-pole filters created using a non-linear biquad filter structure. These non-linearities cause the resonance of the filter to decrease as the signal level increases (see below).
Available in **Lowpass**, **Highpass**, **Bandpass**, **Notch** and **Allpass (Effect)** types.

![Illustration 75: Resonance warp frequency response](/images/manual-xt/Pictures/reso_warp.png)

For more information on the Cutoff Warp filter, you can see [this blog post](https://jatinchowdhury18.medium.com/complex-nonlinearities-episode-5-nonlinear-feedback-filters-115e65fc0402) by Jatin Chowdhury,
or [this 2020 DAFx paper](https://dafx2020.mdw.ac.at/proceedings/papers/DAFx2020_paper_3.pdf), specifically section 3.

Sub-types for **Cutoff Warp** and **Resonance Warp**:

1. **1 Stage tanh** - Output taken from 1<sup>st</sup> stage (2-pole), using `tanh` non-linearities.
2. **2 Stages tanh** - Output taken from 2<sup>nd</sup> stage (4-pole), using `tanh` non-linearities.
3. **3 Stages tanh** - Output taken from 3<sup>rd</sup> stage (6-pole), using `tanh` non-linearities.
4. **4 Stages tanh** - Output taken from 4<sup>th</sup> stage (8-pole), using `tanh` non-linearities.
5. **1 Stage Soft Clip** - Output taken from 1<sup>st</sup> stage (2-pole), using soft-clipping non-linearities.
6. **2 Stages Soft Clip** - Output taken from 2<sup>nd</sup> stage (4-pole), using soft-clipping non-linearities.
7. **3 Stages Soft Clip** - Output taken from 3<sup>rd</sup> stage (6-pole), using soft-clipping non-linearities.
8. **4 Stages Soft Clip** - Output taken from 4<sup>th</sup> stage (8-pole), using soft-clipping non-linearities.
9. **1 Stage OJD** - Output taken from 1<sup>st</sup> stage (2-pole), using OJD non-linearities.
10. **2 Stages OJD** - Output taken from 2<sup>nd</sup> stage (4-pole), using OJD non-linearities.
11. **3 Stages OJD** - Output taken from 3<sup>rd</sup> stage (6-pole), using OJD non-linearities.
12. **4 Stages OJD** - Output taken from 4<sup>th</sup> stage (8-pole), using OJD non-linearities.

**Tri-Pole** - 3-pole filter based on the [Threeler filter](http://ijfritz.byethost4.com/Threeler_board_doc.pdf)
designed by Ian Fritz, which contains 3 filter stages and a resonance stage in a global feedback loop.
...
For more information on the development of the Tri-Pole filter, see
...
**Allpass** - As its name suggests, this filter passes all frequencies equally in gain. However, it is useful to alter the phase relationship in the spectrum.
Unless feedback is involved, its effect can mostly be heard when the Cutoff frequency is in movement. Thus, modulation can be used to get interesting results.
...
Bonsai is an organic-sounding distortion effect, combining a highly non-linear bass boost with an
emphasis-filtered waveshaper and noise AM. It excels at a wide range of very beefy gnarly sounds, but can
sound very good at subtle settings too.
...
Impulse response convolution. Useful for creating spatial reverbs, resonant bodies, and speakers, but can be used for any arbitrary FIR filter.

To load an impulse response, click on the box that says "No impulse response loaded!" You can pick one of the factory impulses from the drop-down menu or load your own. Any loaded impulse response will be copied to the `IMPORTED` directory under the user content directory. You can also drag-and-drop a file into the box, which will load it as an impulse response. Supported file formats are WAV, AIFF, and FLAC, with 1 or 2 channels.

Like wavetables, impulse responses are embedded into the patch file.

|               |                                                                                                   |                                        |
| ------------- | ------------------------------------------------------------------------------------------------- | -------------------------------------- |
| Pre-Delay     | Adds a delay to the incoming sound before convolution begins.                                     | 0 .. 1 sec                             |
| Size*         | Resamples the IR to the given multipler, changing the effective "size" of the space.              | 50 .. 200 %                            |
| Start*        | Distance into the IR to start the convolution, cutting off the beginning parts.                   | 0 .. 90 %                              |
| Reverse From* | Flips the IR starting from the beginning, to the end (0%) or up to halfway through (50%) it.      | 0 .. 50 %, can disable                 |
| Tilt Center   | Center frequency of the integrated tilt EQ.                                                       | 13.75 .. 25087.71 Hz, can disable      |
| Tilt Slope    | Slope of the integrated tilt EQ; negative values slope downwards to the left.                     | -12 .. +12 dB                          |
| Low Cut       | Cutoff frequency of built-in highpass filter.                                                     | 13.75 .. 25087.71 Hz, can disable      |
| High Cut      | Cutoff frequency of built-in lowpass filter.                                                      | 13.75 .. 25087.71 Hz, can disable      |
| Mix           | Blend control between the dry and the wet signal.                                                 | -100 .. +100%                          |

Unlike most other Surge effects, not every parameter here can be modulated. Parameters marked with an asterisk are not modulatable, and can only be changed manually. Some DAWs may be able to automate them, but we do not guarantee a functional sound if you do so.

The Convolution effect is not available in the Surge XT Effects plugin.
...
The Audio Input provides the same functionality as the Audio In oscillator algorithm, but allows you to insert audio
into the effects stage, with is downstream from other voice processing components such as the filters and the waveshaper.
...
![Illustration 80: Wavetable Script Editor](/images/manual-xt/Pictures/wavetable_script_editor.png)

The Wavetable Script Editor (WTSE) is a Lua-powered scripting environment for procedurally generating wavetables. Unlike the Formula modulator, which runs continuously on each sample block in real time, the WTSE outputs only when you press Generate. The result is a new wavetable loaded directly into the corresponding Wavetable oscillator, which can then sweep through its frames via the `Morph` parameter like any other wavetable.

To open an oscillator's WTSE instance, choose the **Wavetable Script Editor...** menu item from the Wavetable or WT Oscillator context menus or use the keybinding (**Alt/Option + W** by default).

A Wavetable Script consists of two functions:
`init(wt)` is called once when the script is first loaded. Use it to set up any data that remains constant across all frames: phase arrays, lookup tables, names, etc. The `wt` table passed into `init` already contains:

|                 |                                         |            |
| --------------- | --------------------------------------- | ---------- |
| wt.frame_count  | Total number of frames in the wavetable | 1 .. 256   |
| wt.sample_count | Number of samples per frame             | 32 .. 4096 |

You may add any fields you like to `wt` and return it; everything you store here will be available in every subsequent generate call. An optional field is `wt.name` to set the Wavetable display name for the Oscillator.

`generate(wt)` is called once per frame when Generate is pressed. It receives the same `wt` table returned by `init`, plus one additional field:

|          |                                      |          |
| -------- | ------------------------------------ | -------- |
| wt.frame | The current frame number (1-indexed) | 1 .. 256 |

`generate` must return a Lua array (table) of exactly `wt.sample_count` numeric values, representing the waveform for the current frame. Values should be in the −1 to 1 range; the engine does not automatically normalize output.

### Prelude and Functions

A wavetable-specific version of the Prelude is available for Wavetable Scripts. This provides built-in utilities including:

`math.linspace(start, stop, n)` generates an evenly spaced array of `n` values from `start` to `stop`, useful for constructing phase arrays in init.

Standard math functions such as `sin`, `cos`, `pi`, `exp`, and `log` are available in the global scope without the `math.` table prefix.

### Workflow

- Write or edit your script in the editor
- Press Apply to validate the script and preview the wavetable without generating
- Press Generate to run `init` once, then call `generate` once per frame, collecting the results into a complete wavetable
- The resulting wavetable is immediately loaded into the active Wavetable oscillator
- The script can be saved with a Surge patch, exported as metadata inside a `.wt` or `.wav` file or saved as a standalone `.wtscript` preset file from the hamburger menu

The graph in the UI can be switched between single-frame and filmstrip mode to preview the wavetable. Dragging the graph moves through frames; if the script is unchanged, frames are loaded from cache, otherwise `generate` is called for each frame as needed.

### Wavetable Snapshots

Snapshots let a script use existing wavetable data as input. Each oscillator has two snapshot slots that can hold a complete wavetable. When `generate(wt)` runs, populated snapshots are available as a 3D table indexed as `wt.snapshot[slot][frame][sample]` (1-indexed).

In the WTSE hamburger menu, the **Import wavetable data** submenu shows the two snapshot slots. For each slot you can capture the current wavetable from any of the three wavetable oscillators in the same scene, or load a `.wt` or `.wav` file from disk. Populated slots show their frame and sample count, along with a clear button to remove the data.

Snapshots are saved inside `.wtscript` preset files and can also be saved inside Surge patches. To include snapshots in a patch file, enable **Save wavetable script snapshots** in the Save Patch dialog.

### FFT Functions

The WTSE exposes four FFT functions built on the PFFFT library. Because wavetable generation is offline (not real-time), these are well suited for spectral analysis, filtering, and resynthesis.

Real transforms operate on a flat array of real-valued samples:

`real_fft(t)` performs a forward real FFT on table `t`, returning a frequency-domain table of the same length.

`real_ifft(t)` performs the inverse, converting a frequency-domain table back to time-domain samples. The output is unscaled; use `scale_real(t)` to normalize it back to the original amplitude (this divides each element by `N`, the FFT size).

Complex transforms operate on interleaved real/imaginary pairs, so a table of length `2N` represents `N` complex values:

`complex_fft(t)` performs a forward complex FFT.

`complex_ifft(t)` performs the inverse complex FFT. As with the real inverse, the output is unscaled; use `scale_complex(t)` to normalize it back to the original amplitude.

All four functions share the same constraints on the input table: it must be a power-of-two length, with a minimum of 32 and a maximum of 524,288 elements. Every element must be a number. The returned table is always the same length as the input.

For the real FFT, the output table layout is:
...
The complex FFT layout follows the same interleaved pattern:
...
| ...   | ...               |
| 2N-1  | Bin N-1 real      |
| 2N    | Bin N-1 imaginary |

### Tips and Techniques

- Additive synthesis is a natural fit: Iterate over partials in generate, adding more complexity in higher frames to create a wavetable that morphs from simple to complex
- Pre-computing in `init` keeps `generate` fast, since `init` runs only once. Store phase arrays, harmonic series, windowing functions there
- Because the script is not real-time, you can afford more expensive computations than would be possible in the Formula modulator: Nested loops and table operations are fine
- See the included tutorial scripts for more in-depth examples
