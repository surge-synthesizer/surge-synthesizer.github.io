...
|                        |                                                                                                |                                       |
| ---------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------- |
| **amplitude**          | Value of the modulator's Amplitude parameter                                                   | Ranges from 0 to 1 or -1 to 1         |
...
| **is_playing**         | Is set to true when the plugin host (DAW) is playing. Always true in standalone                |                                       |
| **is_recording**       | Is set to true when the plugin host (DAW) is recording. Always true in standalone              |                                       |
...
| **labels**             | Optional table for renaming the Rate, Phase, Deform and Amplitude sliders, set in **init**     | See below                             |
...
| **lfo_id**             | Index number of this Formula's LFO slot                                                        | 1-6 for voice, 7-12 for scene LFOs    |
| **macros**             | See tutorial _Macros And Voice Parameters_ (#6)                                                |                                       |
| **mpe_bendrange**      | MPE Pitch bend range                                                                           | Set in the MPE menu                   |
| **mpe_enabled**        | Is set to true if MPE is enabled in the MPE settings sub-menu                                  |                                       |
...
| **released**           | Is set to true when the voice is in the release state                                          |                                       |
...
| **subscriptions**      | Deprecated in Surge XT version 1.4.0                                                           |                                       |
| **sustain**            | Value of the modulator's Sustain parameter                                                     | Ranges from 0 to 1                    |
| **tempo**              | Host tempo                                                                                     |                                       |
| **use_amplitude**      | Set to false to disable the Amplitude slider scaling the output                                |                                       |
| **use_envelope**       | Set to false to disable the LFO Envelope Generator shaping the modulator output                |                                       |
| **use_rate**           | Set to false to disable the ghost waveform in the LFO display when the Rate slider is disabled |                                       |
...
A Formula modulator can also rename the four main LFO sliders. Assigning a **labels** table in the modulator's **init**
function changes the default Rate, Phase, Deform and Amplitude names, both on the sliders and in the modulation matrix.
Every entry is optional, so any slider you leave out keeps its default name:

```lua
function init(state)
    state.labels = { rate = "Foo", phase = "Bar" }
    return state
end
```
